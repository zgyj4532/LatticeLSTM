...
This repository targets Python 2.7 and PyTorch 0.3.0, so uv can only manage the environment if you already have a compatible Python 2.7 interpreter available locally.

1. Install uv.
2. Create or select a Python 2.7 interpreter.
3. From the repository root, run `uv sync` to install the declared dependencies from `pyproject.toml`.
4. If uv cannot resolve Python 2.7 or `torch==0.3.0` on your machine, you will need to either provide a legacy interpreter manually or port the project to Python 3 first.
